...
_STRUCTURE_WORDS = {"estas", "vas", "a", "voy", "yo"}
...
def _content_word(expected_norm: str) -> str:
    ...
    for tok in reversed(_tokens(expected_norm)):
...
def grade_regular(verb: dict, tense: str, transcript: str, strict: bool = False) -> dict:
    ...
    expected_word = _content_word(expected_norm)
    if _has_stem_overlap(expected_word, actual_tokens) or _fuzzy_ratio(expected_norm, actual_norm) >= 0.55:
...
def grade_gustar(item: dict, transcript: str, strict: bool = False) -> dict:
    ...
    if _fuzzy_ratio(expected_norm, actual_norm) >= 0.55:
        result["level"] = "conjugation_error"
